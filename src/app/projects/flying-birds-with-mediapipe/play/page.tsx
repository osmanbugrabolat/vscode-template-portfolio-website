...
export default function FlyingBirdsPlayPage() {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === "Escape") {
        if (isFullscreen) {
          ...
          router.push("/projects/flying-birds-with-mediapipe");
        }
      }
    };
  ...
  const gameContent = (
    <div className={isFullscreen 
      ...
      <div className={`absolute top-4 left-4 z-10 flex gap-4 ${isFullscreen ? 'top-6 left-6' : ''}`}>
        <button 
          onClick={() => router.push("/projects/flying-birds-with-mediapipe")}
      ...
      <iframe
        src="https://osmanbugrabolat.github.io/flying-birds-with-mediapipe/"
        className="w-full h-full border-0 outline-none"
        allow="camera; microphone; autoplay; fullscreen"
        title="Flying Birds with Mediapipe"
      />
    </div>
  );
